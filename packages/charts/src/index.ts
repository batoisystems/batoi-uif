...
function svg(width: number, height: number, content: string): string {
  return `<svg class="uif-chart-svg" viewBox="0 0 ${width} ${height}" role="img" aria-hidden="false">${content}</svg>`;
}
...
function renderBars(data: ChartDatum[], width: number, height: number): string {
  ...
  return data
    .map((d, i) => {
      ...
      const y = height - h - 16;
      return `<rect x="${x}" y="${y}" width="${barWidth}" height="${h}" rx="3"></rect>`;
    })
    .join('');
}
...
export function renderChart(data: ChartDatum[], options: ChartOptions = {}): string {
  ...
  if (type === 'line' || type === 'sparkline') return svg(width, height, renderLine(normalized, width, height));
  if (type === 'area') return svg(width, height, renderLine(normalized, width, height, true));
  if (['pie', 'donut', 'ring', 'progress', 'gauge'].includes(type)) {
    ...
    return svg(120, 120, `<circle cx="60" cy="60" r="45"></circle><circle class="uif-chart-value" cx="60" cy="60" r="45" stroke-dasharray="${dash} 283"></circle>`);
  }
  if (type === 'timeline' || type === 'status-heatmap') return svg(width, height, renderBars(normalized, width, height));
